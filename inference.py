...
if __name__ == '__main__':
    # 1. 설정 (경로 및 하이퍼파라미터)
    # 저장된 체크포인트 파일 경로
    CHECKPOINT_PATH = "./saved/16/checkpoints/checkpoint_50.pth"
    ...
    model = UNet([1,64,128,256,512,1024], device=device).to(device)
    ...
    model.eval()
    w_before = model.U_down[0].DC[0].W.clone()
    ckpt = torch.load(CHECKPOINT_PATH, map_location=device)
    model.load_state_dict(ckpt["model_state"] if "model_state" in ckpt else ckpt)
    w_after = model.U_down[0].DC[0].W
    ...
    output_img = denoised_output.squeeze().detach().numpy()
